...
module.exports = (elements) => {
  ...
  if (elements.menu && elements.trigger && elements.topBar) {
    document.addEventListener('click', (e) => {
      ...
      const isWithinAccMenu = closest(target, '.dqpl-account-item');

      if (isWithinAccMenu) { return; }

      if (!isWithinAccMenu) {
        const accountMenu = document.querySelector('[data-dropdown-expanded]');
        if (accountMenu) {
          const isOpen = accountMenu.getAttribute('data-dropdown-expanded') === 'true';
          if (isOpen) {
            const dropdown = document.querySelector('.dqpl-dropdown-active');
            accountMenu.setAttribute('data-dropdown-expanded', 'false');

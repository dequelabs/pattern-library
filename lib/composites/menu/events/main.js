...
module.exports = (elements) => {
  /**
   * Listen for clicks outside the sidebar and topBar
   * and close them if open AND locked
   */
  ...
  if (elements.menu && elements.trigger && elements.topBar) {
    document.addEventListener('click', (e) => {
      ...
      if (!isWithinDropdown && expandedDropdownItem) {
        expandedDropdownItem.click();
      }

      if (isWithinSideBar || isHamburger || isLocked) {
        return;
      }
